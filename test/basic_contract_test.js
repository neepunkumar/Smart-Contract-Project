...
describe("CarPooling", function () {
    ...
    it("should allow users to query rides", async function () {
        ...
        // Create three rides
        await carPooling.connect(user1).driverRegister();
        await carPooling.connect(user1).createRide(10, 5, ethers.utils.parseEther("10"), 0, 1);
        await carPooling.connect(user2).driverRegister();
        await carPooling.connect(user2).createRide(11, 3, ethers.utils.parseEther("10"), 0, 1);
        await carPooling.connect(user3).driverRegister();
        await carPooling.connect(user3).createRide(12, 2, ethers.utils.parseEther("10"), 1, 2);
    ...
    it("should allow passengers to join rides", async function () {
        ...
        // Create a ride
        await carPooling.connect(user1).driverRegister();
        await carPooling.connect(user1).createRide(10, 2, ethers.utils.parseEther("10"), 0, 1);
        // Register user as a passenger
        ...
        await expect(carPooling.connect(user2).joinRide(0, {value: ethers.utils.parseEther("9")})).to.be.reverted;
        // Check: Registered passenger should be able to join a ride with valid parameters and emit RideJoined event
        balanceBefore = await ethers.provider.getBalance(carPooling.address);
        await expect(carPooling.connect(user2).joinRide(0, {value: ethers.utils.parseEther("10")})).to.emit(carPooling, 'RideJoined').withArgs(0, user2.address);
        balanceAfter = await ethers.provider.getBalance(carPooling.address);
        // Check: Joined ride should transfer funds to contract
        expect(balanceAfter).to.equal(balanceBefore + ethers.utils.parseEther("10"));
        await carPooling.getRideById(0).then((ride) => {
        ...
        expect(await carPooling.connect(user3).joinRide(0, {value: ethers.utils.parseEther("10")})).to.emit(carPooling, 'RideJoined').withArgs(0, user3.address);
        await carPooling.getRideById(0).then((ride) => {
